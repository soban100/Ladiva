...
import { updateProduct, getCategories } from '../../services/productService';
...
export const EditProductModal: React.FC<EditProductModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use the base64 data URL directly (no storage upload needed)
      const imageUrl = formData.image || product.image;

      // Find category ID from category name
      const category = categories.find(cat => cat.name === formData.category);
      const categoryId = category?.id || product.category;
      ...
      const updatedProduct = {
        ...
        status: !formData.stock || parseInt(formData.stock) === 0 ? 'out_of_stock' : 'active',
      };
      ...
      if (result.success) {
        ...
        error('Failed to update product', result.error || 'Unknown error occurred');
      }
    } catch (err) {
      console.error('❌ [EDIT PRODUCT] Unexpected error:', err);
      error('Failed to update product', 'An unexpected error occurred');
    } finally {
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500">
                    {categories.find(c => c.name === formData.category)?.name || 'Category'} • 
                    Rs.{formData.price || '0'} • 
